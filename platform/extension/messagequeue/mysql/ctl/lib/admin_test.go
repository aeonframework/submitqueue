...
func TestInspectMessage(t *testing.T) {
	...
	mock.ExpectQuery("SELECT .+ FROM queue_messages WHERE tenant = \\? AND topic = \\? AND id = \\?").
		WithArgs("acme", "orders", "msg-1").
		WillReturnRows(rows)

	detail, found, err := store.InspectMessage(context.Background(), "acme", "orders", "msg-1")
	require.NoError(t, err)
	assert.True(t, found)
...
func TestInspectMessageNotFound(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"tenant", "offset", "id", "topic", "partition_key", "created_at", "published_at", "payload", "metadata", "failed_at", "failure_count", "last_error", "original_topic"})
	mock.ExpectQuery("SELECT .+ FROM queue_messages WHERE tenant = \\? AND topic = \\? AND id = \\?").
		WithArgs("acme", "orders", "missing").
		WillReturnRows(rows)

	_, found, err := store.InspectMessage(context.Background(), "acme", "orders", "missing")
	require.NoError(t, err)
...
func TestDeleteMessage(t *testing.T) {
	...
	mock.ExpectExec("DELETE FROM queue_messages WHERE tenant = \\? AND topic = \\? AND id = \\?").
		WithArgs("acme", "orders", "msg-1").
		WillReturnResult(sqlmock.NewResult(0, 1))

	affected, err := store.DeleteMessage(context.Background(), "acme", "orders", "msg-1")
	require.NoError(t, err)
...
func TestRequeueDLQ(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectQuery("SELECT .+ FROM queue_messages WHERE tenant = \\? AND topic = \\? AND id = \\?").
		WithArgs("acme", "orders_dlq", "msg-1").
	...
	err = store.RequeueDLQ(context.Background(), "acme", "orders", "msg-1", "_dlq")
	require.NoError(t, err)
	assert.NoError(t, mock.ExpectationsWereMet())
}
...
func TestListOffsetsFiltered(t *testing.T) {
	...
	offsets, err := store.ListOffsets(context.Background(), "acme", "group-1")
	require.NoError(t, err)
	assert.Len(t, offsets, 1)
	assert.NoError(t, mock.ExpectationsWereMet())
}
...
func TestResetOffset(t *testing.T) {
	...
	mock.ExpectExec("UPDATE queue_offsets SET offset_acked = \\?, updated_at = \\?").
		WithArgs(int64(0), sqlmock.AnyArg(), "acme", "group-1", "orders", "repo-1").
		WillReturnResult(sqlmock.NewResult(0, 1))
...
func TestListLeases(t *testing.T) {
	...
	leases, err := store.ListLeases(context.Background(), "acme")
	require.NoError(t, err)
...
func TestStaleLeases(t *testing.T) {
	...
	leases, err := store.StaleLeases(context.Background(), "acme", 60000)
	require.NoError(t, err)
	assert.Len(t, leases, 1)
	assert.Equal(t, "acme", leases[0].Tenant)
	assert.Equal(t, "worker-1", leases[0].LeasedBy)
