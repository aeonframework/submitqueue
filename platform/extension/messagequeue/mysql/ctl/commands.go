...
func newInspectMessageCmd(store **lib.AdminStore, jsonOut *bool) *cobra.Command {
	var tenant, topic, messageID string
	cmd := &cobra.Command{
		Use:   "inspect-message",
		Short: "Show full message details including payload and metadata",
		RunE: func(cmd *cobra.Command, args []string) error {
			detail, found, err := (*store).InspectMessage(cmd.Context(), tenant, topic, messageID)
			if err != nil {
				return err
			}
			if !found {
...
func newDeleteMessageCmd(store **lib.AdminStore, noInteractive *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			affected, err := (*store).DeleteMessage(cmd.Context(), tenant, topic, messageID)
			if err != nil {
				return err
			}
			if affected == 0 {
...
func newRequeueDLQCmd(store **lib.AdminStore) *cobra.Command {
	var tenant, topic, messageID, dlqSuffix string
	cmd := &cobra.Command{
		Use:   "requeue-dlq",
		Short: "Move a DLQ message back to its original topic",
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := (*store).RequeueDLQ(cmd.Context(), tenant, topic, messageID, dlqSuffix); err != nil {
				return err
			}
...
func newListOffsetsCmd(store **lib.AdminStore, jsonOut *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			offsets, err := (*store).ListOffsets(cmd.Context(), tenant, consumerGroup)
			if err != nil {
				return err
			}
			if *jsonOut {
...
func newListLeasesCmd(store **lib.AdminStore, jsonOut *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			leases, err := (*store).ListLeases(cmd.Context(), tenant)
			if err != nil {
				return err
			}
			if *jsonOut {
...
func newStaleLeasesCmd(store **lib.AdminStore, jsonOut *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			leases, err := (*store).StaleLeases(cmd.Context(), tenant, thresholdMs)
			if err != nil {
				return err
			}
			if len(leases) == 0 {
