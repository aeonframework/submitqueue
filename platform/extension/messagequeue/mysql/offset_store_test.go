...
func TestOffsetStore_Initialize(t *testing.T) {
	...
	mock.ExpectExec("INSERT IGNORE INTO queue_offsets").
		WithArgs(testTenant, testConsumerGroup, topic, partitionKey, sqlmock.AnyArg()).
...
func TestOffsetStore_UpdateAckedOffset(t *testing.T) {
	...
	mock.ExpectExec("UPDATE queue_offsets").
		WithArgs(offset, sqlmock.AnyArg(), testTenant, testConsumerGroup, topic, partitionKey, offset).
	...
	}{
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec("DELETE FROM queue_offsets").
					WithArgs(testTenant, testConsumerGroup, "test_topic", "part-1").
					WillReturnResult(sqlmock.NewResult(0, 1))
			},
		},
		{
