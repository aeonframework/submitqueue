...
func (s *sqloffsetStore) Initialize(ctx context.Context, tenant string, topic string, partitionKey string, consumerGroup string) (retErr error) {
	...
	_, err := s.db.ExecContext(ctx, fmt.Sprintf(`
		INSERT IGNORE INTO %s (tenant, consumer_group, topic, partition_key, offset_acked, updated_at)
		VALUES (?, ?, ?, ?, 0, ?)
	`, OffsetsTableName), tenant, consumerGroup, topic, partitionKey, now)
...
func (s *sqloffsetStore) GetAckedOffset(ctx context.Context, tenant string, topic string, partitionKey string, consumerGroup string) (_ int64, retErr error) {
	...
	err := s.db.QueryRowContext(ctx, fmt.Sprintf(`
		SELECT offset_acked FROM %s WHERE tenant = ? AND consumer_group = ? AND topic = ? AND partition_key = ?
	`, OffsetsTableName), tenant, consumerGroup, topic, partitionKey).Scan(&offset)
...
func (s *sqloffsetStore) UpdateAckedOffset(ctx context.Context, tenant string, topic string, partitionKey string, offset int64, consumerGroup string) (retErr error) {
	...
	_, err := s.db.ExecContext(ctx, fmt.Sprintf(`
		...
		WHERE tenant = ? AND consumer_group = ? AND topic = ? AND partition_key = ? AND offset_acked < ?
	`, OffsetsTableName), offset, now, tenant, consumerGroup, topic, partitionKey, offset)
...
func (s *sqloffsetStore) DeleteOffset(ctx context.Context, tenant string, topic string, partitionKey string, consumerGroup string) (retErr error) {
	...
	_, err := s.db.ExecContext(ctx, fmt.Sprintf(`
		DELETE FROM %s WHERE tenant = ? AND consumer_group = ? AND topic = ? AND partition_key = ?
	`, OffsetsTableName), tenant, consumerGroup, topic, partitionKey)
